...
new_model = VIT_Couple()
new_model.load_state_dict(torch.load('version_test4.pth', map_location=torch.device('cpu')))
new_model.eval()


def make_prediction(img, img1):
    with torch.no_grad():
        img = img.view(1, 1, 3, 224, 224)
        img1 = img1.view(1, 1, 3, 224, 224)
        #img = img.to('cuda')
        #img1 = img1.to('cuda')
        #new_model = new_model.to('cuda')
        outputs = new_model(img, img1)
        #st.write(outputs)
        _, predicted = torch.max(outputs.data, 1)

        return predicted

st.title("img classification")
upload = st.file_uploader(label = "upload img", type = ["png", "jpg", "jpeg"])
upload1 = st.file_uploader(label = "upload img1", type = ["png", "jpg", "jpeg"])

if upload and upload1:
    img = Image.open(upload).convert("RGB")
    img1 = Image.open(upload1).convert("RGB")
    img_show, img1_show = st.columns(2)
    with img_show:
        st.image(img, caption='img', width=300)
    with img1_show:
        st.image(img1, caption='img1', width=300)
    ...
    #st.write(x.equal(y))
    prediction = make_prediction(x, y)
    st.header("result")
    if prediction.item() == 1:
        st.write("they are couple avatar")
    else:
        st.write("they are not couple avatar")
    #st.write(prediction)
